...
class Train
{
	...
	function count_cars()
	{
		...
		{
			$car_count++;
			$next = $next->next; //move to next car
		}
	...
	function add_first_car($weight)
	{
		if($this->count_cars() < 30) //less than 30 cars in train; add new one
		{
			...
			{
				$new_car->next = $this->first_car; //set former head as new head's next
				$this->first_car->previous = $new_car; //old puts new -first as previous
	...
	function add_last_car($weight)
	{
		if($this->count_cars() < 30) //less than 30 cars in train; add new one
		{
			...
			{
				$new_car->previous = $this->last_car; //set former last as new last's previous
				$this->last_car->next = $new_car; //set new last as former last's next
	...
	function remove_first()
	{
		...
		{
			$this->first_car = $this->first_car->next;
			$this->first_car->previous = null;
		}
	}
	...
	function remove_last()
	{
		...
		{
			$this->last_car = $this->last_car->previous;
			$this->last_car->next = null;
		}
	}
}
